...
import com.deetechs.wordwiz.Contact;
import com.deetechs.wordwiz.DatabaseHandler;
...
* @author Benjamin
*/
public class ResumePlayActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //set values from WordWizzApp Activity
        Bundle extras = getIntent().getExtras(); 
        Log.d("Intent: ","Get Intent done");
        if(extras !=null)
        {           
            count = extras.getInt("counter");
            level = extras.getInt("levels");
            wrong_tracker = extras.getInt("wrongs");
         }       
        ...
        btn_submit.setOnClickListener(new View.OnClickListener() { 
            public void onClick(View arg0) {
            	...
            	Log.d("Resume", "Am Resume play: word length!!!"+user_entry.length());
            	Log.d("Resume", "Word From XML "+word);
                Log.d("Resume", "Word From user "+user_entry);
                Log.d("Resume", "Count: "+count);
                Log.d("Resume", "Wrong: "+wrong_tracker);
                //handle empty words
                if(user_entry.length()==0){
                	// Creating alert Dialog with one button for empty fields
                	myDialog = new Dialog(ResumePlayActivity.this);
                ...
                //word is right OR wrong
            		...
            		if(count<20 && user_entry.equalsIgnoreCase(word)){                			
                    	...
                    	public void onClick(View v) {
                        	...
                        	Intent play = new Intent(getApplicationContext(), ResumePlayActivity.class);              	
                        	startActivity(play);  
                    		myDialog.dismiss();
                    			}
                    		});		                    	
            		...
            		if(count<20 && !user_entry.equalsIgnoreCase(word)){            			
            			wrong_tracker++;                    	                   		
                    	Intent wrong = new Intent(getApplicationContext(), WrongResumeActivity.class);              	
                    	startActivity(wrong);  
            		}
            		if(count==21 && points<12){                			
            		 ...
            		 if(count==21 && points>=12){                			
            			level++;                		
            			Intent level_separator = new Intent(getApplicationContext(), LevelSepResumeActivity.class);              	
